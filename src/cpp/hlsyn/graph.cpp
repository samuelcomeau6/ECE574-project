...
Graph::Graph(const Graph &g){
    ...
    for(int i=0; i < g.nodes.size(); ++i){
        ...
        for(int j = 0; j < this->edges.size(); ++j){
            ...
            if(node->input_1->name == edge->name) {
                in1=edge;
            }
            if(node->input_2->name == edge->name) {
                in2=edge;
            }
            if(node->type==MUX2X1){
                    if(node->select->name == edge->name) {
                        sel=edge;
                    }
            }
            if(node->output->name == edge->name) {
                out=edge;
            }
        }
...
node_t * Graph::add_node(comp_t component_type, std::string input1_name, std::string input2_name, std::string select, std::string output_name){
    ...
    if(component_type == MUX2X1) {
        temp_obj->select     = edge_search(this, select, false);
    } else {
...
edge_t * Graph::add_edge(comp_t component_type, std::string name, int data_width, bool is_signed){
    ...
    if(component_type==INPUT) {
        temp_obj->from        = &(this->inop);
    } else {
        temp_obj->from        = NULL;
    }

    if(component_type==OUTPUT || component_type == THEN || component_type == ELSE) {
        temp_obj->to         = &(this->onop);
    } else {
        temp_obj->to         = NULL;
    }
    temp_obj->type           = component_type;
...
std::string Graph::start_graph_toString(void){
    ...
    out += "inop[label = \""+inop.name+"\"]\n";
    out += "onop[label = \""+onop.name+"\"]\n";

    for(int i=0;i<this->nodes.size();++i){
         out += nodes[i]->name + "[label =\"" + node_toString(*this->nodes[i])+ "t:"
         + std::to_string(this->nodes[i]->start_time)+"+"+std::to_string(this->nodes[i]->duration)+ "\"]\n";
    }

    for(int i=0;i<this->edges.size();++i){
        if(this->edges[i]->from != NULL) {
            out += this->edges[i]->from->name;
        } else {
            out += "NULL";
        }
        out += "->";
        if(this->edges[i]->to != NULL) {
            out += this->edges[i]->to->name;
        } else {
            out += "NULL";
        }
        out += "[label =\"" + this->edges[i]->name + "\"]\n";
    }
    return out;
}
...
std::string node_toString(node_t datum){
    ...
    out_string += datum.is_signed ? " S " :" U ";
    out_string +=std::to_string(datum.width) + "' ";
    return out_string;
}
...
std::string edge_toString(edge_t datum){
    std::string out_string = datum.name;
    out_string += datum.is_signed ? " signed " : " unsigned ";
    out_string += std::to_string(datum.width) + " bit ";
    out_string += comp_toString(datum.type);
    out_string += " From:";
    out_string += (datum.from == NULL) ? "NULL" : datum.from->name;
    out_string += " To:";
    out_string += (datum.to == NULL) ? "NULL" : datum.to->name;
    return out_string;
}
edge_t* edge_search(Graph * list, std::string name,bool is_from){
    ...
    if(addable){
        edge_t * new_edge = list->edges[add_index];
        list->add_edge(new_edge->type, new_edge->name, new_edge->width, new_edge->is_signed);

        if(!is_from) {
            list->edges.back()->from = new_edge->from;
        }
        if(is_from) {
            list->edges.back()->to = new_edge->to;
        }
