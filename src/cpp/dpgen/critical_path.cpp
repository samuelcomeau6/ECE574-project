...
void TopSortVisit(data_list * Graph, data_list * List, data_type uVertex) {
	...
	for (int i = 0; i<Graph->data_v.size(); i++) {
		if (Graph->data_v[i].is_operation){
			int compareInput1 = -1;
			int compareInput2 = -1;
			if(uVertex.is_operation && uVertex.output_name) {
				compareInput1 = strcmp(uVertex.output_name,Graph->data_v[i].input_1_name);
				compareInput2 = strcmp(uVertex.output_name,Graph->data_v[i].input_2_name);
			}
			if(!uVertex.is_operation && uVertex.input_1_name){
				compareInput1 = strcmp(uVertex.input_1_name,Graph->data_v[i].input_1_name);
				compareInput2 = strcmp(uVertex.input_1_name,Graph->data_v[i].input_1_name);
			}
			//If uVertex is one of the inputs
			if (compareInput1==0 || compareInput2==0) {
				if (Graph->data_v[i].color == "White"){
					//If node i is the same as uVertex?
					if (strcmp(Graph->data_v[i].operation_name,"REG") != 0) {
						//If node i is not a register
